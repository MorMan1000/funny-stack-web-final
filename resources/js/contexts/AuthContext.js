...
export const AuthProvider = ({ children }) => {
  ...
  const signup = async (userInfo) => {
    try {
      ...
      const userDetails = {
        user: { displayName: userInfo.displayName, userEmail: userInfo.userEmail, userPassword: userInfo.userPassword }, redirect: "https://funny-stack.herokuapp.com/api/auth/verify-account/"
      };
      const options = {
  ...
  const resetPassword = async (email) => {
    setLoading(true);
    const response = await axios.post("/api/auth/reset-password", { userEmail: email, redirect: "/reset-password", redirect: "https://funny-stack.herokuapp.com/reset-password" });
    setLoading(false);
